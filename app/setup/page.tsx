          ...
          <section>
            ...
            <div className="mt-8 flex flex-col gap-3 sm:flex-row sm:items-center">
              <button
                ...
                disabled={isSaving}
                className="flex flex-1 items-center justify-center gap-2 rounded-md bg-green-600 px-6 py-3.5 text-sm font-medium text-white hover:bg-green-700 focus:outline-none focus:ring-2 focus:ring-green-500 focus:ring-offset-2 disabled:opacity-60"
              >
                {isSaving && <Spinner />}
                Start Review →
              </button>
              <button
                onClick={() => { window.location.href = "/api/export-xact"; }}
                className="flex items-center justify-center gap-2 rounded-md bg-blue-600 px-6 py-3.5 text-sm font-medium text-white hover:bg-blue-700 focus:outline-none focus:ring-2 focus:ring-blue-500 focus:ring-offset-2"
              >
                <svg className="h-4 w-4" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
                  <path strokeLinecap="round" strokeLinejoin="round" d="M4 16v1a3 3 0 003 3h10a3 3 0 003-3v-1m-4-4l-4 4m0 0l-4-4m4 4V4" />
                </svg>
                Export XactContents .xls
              </button>
            </div>
          </section>
        )}
