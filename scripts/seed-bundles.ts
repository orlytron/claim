...
/*
  Run this SQL in Supabase dashboard BEFORE running this script:
  ...
  CREATE TABLE IF NOT EXISTS bundles (
    ...
    plausibility text NOT NULL DEFAULT 'green',
...
const BUNDLES: Bundle[] = [
  ...
  // ── BEDROOM RAFE ──────────────────────────────────────────────────────────────
  {
    ...
    items: [
      { description: "1994 MLB complete print run collection — all major sets Topps Upper Deck Fleer Donruss Score Bowman Finest SP", brand: "DeBasel", qty: 1, unit_cost: 14000, total: 14000, category: "Collectibles" },
  ...
  },
  {
    ...
    items: [
      { description: "1994 MLB complete print run collection all major sets", brand: "DeBasel", qty: 1, unit_cost: 14000, total: 14000, category: "Collectibles" },
...
async function main() {
  console.log(`Seeding ${BUNDLES.length} bundles…`);

  const { error } = await supabase.from("bundles").upsert(BUNDLES, {
  ...
  console.log(`✓ Seeded ${BUNDLES.length} bundles successfully.`);

  // Verify
  const { data, error: fetchErr } = await supabase
    .from("bundles")
    .select("bundle_code, room, name, total_value")
    .order("room");

  if (fetchErr) {
    console.error("Verify failed:", fetchErr);
  } else {
    console.log("\nBundles in database:");
    data?.forEach((b) => {
      console.log(`  ${b.bundle_code.padEnd(10)} ${b.room.padEnd(30)} ${b.name} — $${b.total_value.toLocaleString()}`);
    });
  }
}
